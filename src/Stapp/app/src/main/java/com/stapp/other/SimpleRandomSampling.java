...
public class SimpleRandomSampling {
  ...
  public String questiondescription() {
    String des =
        "Registrar office want to estimate the height of all females on campus. The total number of female is "
            + this.data.length
            + "."
  ...
  public boolean elementexist(int[] ex, int index) {
    for (int i = 0; i < ex.length; i++) if (ex[i] == index) return true;
    return false;
  }
